...
public class PauseMenuDisplay extends BaseScreenDisplay {
    ...
    protected void buildUI(Table root) {
        ...
        panel.defaults().pad(10f).center();
        panel.add(button("Resume", 2f, () -> entity.getEvents().trigger("resume"))).row();
        panel.add(button("Restart", 2f, () -> game.setScreen(GdxGame.ScreenType.MAIN_GAME))).row();
        panel.add(button("Main Menu", 2f, this::backMainMenu)).row();
        panel.add(button("save", 2f, () -> {
