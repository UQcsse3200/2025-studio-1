...
import com.csse3200.game.components.RoomIdComponent;
...
public abstract class GameArea implements Disposable {
    ...
    /**
     * Each area/room must identify itself.
     */
    public String getRoomId() {
        String simple = getClass().getSimpleName();
        return simple.endsWith("GameArea")
                ? simple.substring(0, simple.length() - "GameArea".length())
                : simple;
    }

    public Entity getPlayer() {
        return ServiceLocator.getPlayer(); // if you always sync ServiceLocator in spawnPlayer
    }

    public final Entity acceptOrSpawnPlayer(String roomId, GridPoint2 cell, boolean centerOnTile) {
        Entity player = ServiceLocator.getPlayer();
        if (player == null) {
            // First room: create and register once
            player = PlayerFactory.createPlayer();
            ServiceLocator.registerPlayer(player);
        } else {
            // Ensure not double-registered across transitions
            ServiceLocator.getEntityService().unregister(player);
        }

        // (Optional) tag or update room id component
        player.addComponent(new RoomIdComponent(roomId));

        // Drop into this area
        spawnEntityAt(player, cell, centerOnTile, centerOnTile);
        return player;
    }

    public final Entity acceptOrSpawnPlayer(PlayerSpawnSpec spec) {
        return acceptOrSpawnPlayer(spec.roomId(), spec.cell(), spec.centerOnTile());
    }


    public final Entity spawnPlayer(PlayerSpawnSpec spec) {
        Entity player = spec.playerFactory().get();
        // Optional: tag with room id if you use it elsewhere
        player.addComponent(new RoomIdComponent(spec.roomId()));

        // centerOnTile → apply to both X and Y (your API splits them)
        spawnEntityAt(player, spec.cell(), /*centerX=*/spec.centerOnTile(), /*centerY=*/spec.centerOnTile());

        // Optional: keep ServiceLocator in sync
        if (ServiceLocator.getPlayer() == null) ServiceLocator.registerPlayer(player);

        return player;
    }

    public void teleportPlayer(String targetRoomId, GridPoint2 cell) {
        var terrainFactory = this.terrainFactory;
        var cameraComponent = this.cameraComponent;

        clearAndLoad(() -> {
            AreaRouter router = ServiceLocator.getAreaRouter();
            GameArea next = router.create(targetRoomId, terrainFactory, cameraComponent);
            next.create();
            next.spawnPlayer(PlayerSpawnSpec.of(next.getRoomId(), cell));
            return next;
        });
    }
    ...
    protected boolean beginTransition() {
        if (isTransitioning || ServiceLocator.getTransitioning()) return false;
        isTransitioning = true;
        ServiceLocator.registerTransitioning(true);
        return true;
    }
    ...
    protected void endTransition() {
        ...
        ServiceLocator.registerTransitioning(false);
    }
    ...
    protected void spawnEntityAt(
        ...
        float tileHalfSize = terrain.getTileSize() * 0.5f;

        if (centerX) worldPos.x += tileHalfSize;
        if (centerY) worldPos.y += tileHalfSize;

        entity.setPosition(worldPos);
        spawnEntity(entity);
    }

    public final Entity ensurePlayer(PlayerSpawnSpec spec) {
        // create or reuse the global player
        Entity global = ServiceLocator.getPlayer();
        if (global == null) {
            global = spec.playerFactory().get();
            // register the player with entity service
            ServiceLocator.getEntityService().register(global);
            ServiceLocator.registerPlayer(global);
        }

        // tag/update room id
        RoomIdComponent id = global.getComponent(RoomIdComponent.class);
        if (id == null)
            global.addComponent(new RoomIdComponent(spec.roomId()));
        else
            id.setRoomId(spec.roomId());

        // position the player
        Vector2 worldPos = terrain.tileToWorldPosition(spec.cell());
        float half = terrain.getTileSize() * 0.5f;
        if (spec.centerOnTile()) worldPos.add(half, half);

        global.setPosition(worldPos);
    ...
    protected void clearAndLoad(Supplier<GameArea> nextAreaSupplier) {
        ...
        final Entity player = ServiceLocator.getPlayer();

        for (Entity entity : areaEntities) entity.setEnabled(false);
        ...
        Gdx.app.postRunnable(() -> {
            ...
            if (player != null) {
                try { entityService.unregister(player); } // no-op if not registered
                catch (Exception ignored) {
                    // safe-guard: different implementations may throw if not present
                }
            }

            // Dispose everyone else
            for (Entity e : new java.util.ArrayList<>(areaEntities)) {
                if (e != player) {
                    e.dispose();
                }
            }
            areaEntities.clear(); // Next area will repopulate; player is preserved separately
