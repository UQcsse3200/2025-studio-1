...
public abstract class GameArea implements Disposable {
    ...
    public void spawnEnemies(int roomNumber, int total, float scaleFactor, Entity player) {
        ...
        switch (roomNumber) {
            ...
            case 6, 7:
                ...
                break;
            default:
                throw new IllegalStateException("Unexpected room number: " + roomNumber);
        }
    }
    ...
    /**
     * Teleport an existing player entity to the spawn point for the given room.
     * Returns true on success.
     */
    public boolean teleportPlayerToRoom(Entity player, String roomName) {
        if (player == null || roomName == null || roomName.isEmpty()) return false;
        ...
        // Optional: nudge the camera to follow immediately
        try {
            if (cameraComponent != null && cameraComponent.getEntity() != null) {
                cameraComponent.getEntity().setPosition(pos);
            }
        } catch (Exception ignored) {
            // Camera is optional; fail-soft
        }

        Gdx.app.log("GameArea", "Teleported player to " + roomName + " at " + pos);
        return true;
    }

    /**
     * Returns a concise room id derived from the concrete GameArea class name.
     * Examples:
     *   "Floor3GameArea" -> "3"
     *   "Room7GameArea"  -> "7"
     *   "MainHallGameArea" -> "MainHall"
     */
    public String getRoomIdFromClassName() {
        String simple = getClass().getSimpleName();
        // strip "GameArea" anywhere in the name (case-insensitive)
        simple = simple.replaceAll("(?i)GameArea", "");
        // try Floor/Room + digits; return just the number (e.g., "Floor3" -> "3")
        java.util.regex.Matcher m =
                java.util.regex.Pattern.compile("(?i)(?:floor|room)\\s*(\\d+)").matcher(simple);
        if (m.find()) return m.group(1);
        // fallback: whatever remains, e.g., "MainHall"
        return simple;
    }

    public static final class RoomBounds {
        public final float minX, maxX, minY, maxY;
        public RoomBounds(float minX, float maxX, float minY, float maxY) {
            this.minX = minX; this.maxX = maxX; this.minY = minY; this.maxY = maxY;
        }
        public com.badlogic.gdx.math.Vector2 center() {
            return new com.badlogic.gdx.math.Vector2((minX + maxX) * 0.5f, (minY + maxY) * 0.5f);
        }
    }
